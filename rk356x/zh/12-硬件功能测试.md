...
kernel-5.10/arch/arm64/boot/dts/rockchip/rk3568-kickpi-k1b-linux.dts
kernel-5.10/arch/arm64/boot/dts/rockchip/rk3568-kickpi-k1-linux.dts
kernel-5.10/arch/arm64/boot/dts/rockchip/rk3562-kickpi-k3-android.dts
kernel-5.10/arch/arm64/boot/dts/rockchip/rk3562-kickpi-k3-linux.dts
```
...
未注册如下：pin 46 (gpio1-14): (MUX UNCLAIMED) (GPIO UNCLAIMED)

步骤四

​		通过 /sys/class/gpio/export 注册 46并进行控制

```
// 注册
root@kickpi:~# echo  46 > /sys/class/gpio/export
// 查看是否生成
root@kickpi:~# ls /sys/class/gpio/
export  gpio46  gpiochip0  gpiochip352  unexport
// 注册后的结点内容
root@kickpi:~# ls /sys/class/gpio/gpio46
...
value
	0 / 1
	cat /sys/class/gpio/gpio46/value 		// 读取
	echo 1 > /sys/class/gpio/gpio46/value	// 配置高电平
	echo 0 > /sys/class/gpio/gpio46/value  // 配置低电平
```
