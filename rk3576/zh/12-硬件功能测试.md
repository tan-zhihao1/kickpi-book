...
```
$ ls /sys/class/leds/
GPIO0_A5  GPIO3_D4  GPIO4_D1    SDMMC0_PWREN  mmc2::
GPIO3_B0  GPIO3_D5  PCIE_PWREN  fan           work
```
...
kickpi开发板调试串口一般内嵌到扩展引脚上，可通过[扩展引脚章节](#ExpansionPin) 查看对应的 DEBUG 引脚位置，一般按顺序排列为 `UART_RX_DEBUG`、`UART_TX_DEBUG`、`GND`。
...
可通过[扩展引脚章节](#ExpansionPin) 查看对应的 ADC 位置及通道号
