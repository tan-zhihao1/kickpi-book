...
K7 支持 CAN FD标准，CAN FD继承了CAN的绝大多数特性，如同样的物理层，双线[串行通信协议](https://zhida.zhihu.com/search?content_id=239082283&content_type=Article&match_order=1&q=串行通信协议&zhida_source=entity)，基于非破坏性仲裁技术，分布式实时控制，可靠的错误处理和检测机制等，同时CAN FD弥补了CAN在总线带宽和数据长度方面的不足。
...
```
$ io -4 0x2ac00000 0x8415
```

>2ac00000 为 K7 CAN0 的 dts 节点
>
>可查看dts节点
>
>find /sys/devices/platform/ | grep can0 | grep uevent
>
>/sys/devices/platform/2ac00000.can/net/can0/uevent
